...
export default {
    ...
    computed: {
        ...
        ...mapState( ['v_repetitive', 'v_repetitive_days', 'v_repetitive_dates', 'v_from_time', 'v_to_time']),
    },
        ...
        schedule_total_days(){
           ...
           // use this condition when user create event
            ...
            if(Object.keys(this.schedules).length > 0)
            {
                ...
                // // use this condition when user edit event
                // // total event's dates in monthly event
                if(this.schedules.repetitive_type == 3  && this.schedules.repetitive_type)
                {
                    var repetitive_dates     = this.schedules.repetitive_dates.split(',');
                ...
                // // use this condition when user edit event
                // // total event's dates in weekly event
                if(this.schedules.repetitive_type == 2  && this.schedules.repetitive_type )
                {
                    var all_dates = [];
                    var i         = 1;
                    var $_this    = this;
