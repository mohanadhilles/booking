        ...
        <div class="row">
            <div class="col-md-3">
                <div class="alert alert-info" role="alert">
                    <small>{{ trans('em.total_bookings') }}</small><br> 
                    <small><strong>{{ total_earning.customer_paid_total }} {{ currency }}</strong></small>
                </div>
            </div>
            <div class="col-md-3">
                <div class="alert alert-danger" role="alert">
                    <small>{{ trans('em.total_admin_commission') }}</small><br> 
                    <small><strong>{{ total_earning.admin_commission_total }} {{ currency }}</strong></small>
                </div>
            </div>
            <div class="col-md-3">
                <div class="alert alert-danger" role="alert">
                    <small>{{ trans('em.total_admin_tax') }}</small><br> 
                    <small><strong>{{ total_earning.admin_tax_total }} {{ currency }}</strong></small>
                </div>
            </div>
            <div class="col-md-3">
                <div class="alert alert-success" role="alert">
                    <small>{{ trans('em.total_profit') }}</small><br> 
                    <small><strong>{{ total_earning.organiser_earning_total }} {{ currency }}</strong></small>
                </div>
            </div>
        </div>
        <hr>
        ...
        <div class="row">
            <div class="col-md-12 table-responsive">
                <table class="table table-hover table-striped">
                    <thead>
                        <tr>
                            <th class="col-xs-2">{{ trans('em.event') }}</th>
                            <th class="col-xs-2">{{ trans('em.bookings') }}</th>
                            <th class="col-xs-2">{{ trans('em.commission') }}</th>
                            <th class="col-xs-2">{{ trans('em.admin_tax') }}</th>
                            <th class="col-xs-2">{{ trans('em.profit') }}</th>
                            <th class="col-xs-2">{{ trans('em.month') }}</th>
                            <th class="col-xs-2">{{ trans('em.transferred') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(earning, index) in event_earning" :key="index" >
                            <td><a :href="eventSlug(earning.event_slug)">{{ earning.event_name }}</a></td>
                            <td>{{ earning.customer_paid_total }} {{ currency }}</td>
                            <td>{{ earning.admin_commission_total }} {{ currency }}</td>
                            <td>{{ earning.admin_tax_total }} {{ currency }}</td>
                            <td>{{ earning.organiser_earning_total }} {{ currency }}</td>
                            <td>{{ moment(earning.month_year, 'MM YYYY').format('MMM ,YYYY') }}</td>
                            <td>
