        ...
        <div class="lgx-tab" v-if="event.repetitive > 0">
            <ul class="nav nav-pills lgx-nav">
                ...
                >
                    <a data-toggle="pill">
                        <h3>
                            <span>{{ index == 0 ? moment(changeDateFormat(local_start_date, "YYYY-MM-DD"), "ll").format('DD') 
                                : moment(changeDateFormat(local_from_date[index], "YYYY-MM-DD"), "ll").format('DD') }} -
                            </span>
                            {{ index == (calculate_months.length-1) ? moment(changeDateFormat(local_end_date, "YYYY-MM-DD"), 'll').format('DD MMM YYYY')
                                : moment(changeDateFormat(local_to_date[index], "YYYY-MM-DD"), "ll").format('DD MMM YYYY') }} 
                        </h3> 
                        <p>
                                        ...
                                        >
                                            <div class="schedule-info" 
                                                @click="
                                                    ...
                                                    changeTimeFormat(local_from_time[index],'hh:mm A'), changeTimeFormat(local_to_time[index], 'hh:mm A') ) 
                                                    : null
                                                ">
                                                <h3 class="title">{{ dateToShortDate(selected_date, schedules[index].from_date) }}</h3>
                                                <h4 class="time">{{changeTimeFormat(local_from_time[index], "hh:mm A") }} - {{ changeTimeFormat(local_to_time[index], 'hh:mm A')}}</h4>
                                    ...
                                    <a role="button" data-toggle="collapse" aria-expanded="true" aria-controls="collapseOne">
                                        <div class="lgx-single-schedule"  
                                            ...
                                            @click="
                                                ...
                                                changeTimeFormat(local_from_time[index],'hh:mm A'), 
                                                changeTimeFormat(local_to_time[index], 'hh:mm A') 
                                            ) : null" 
                                        >
                                    ...
                                    <a role="button" data-toggle="collapse" aria-expanded="true" aria-controls="collapseOne">
                                        ...
                                        >
                                            <div class="schedule-info" 
                                                ...
                                                ? singleEvent() : null"
                                            >
                                                <h3 class="title">{{ convert_date_to_local_format(event.start_date, "ll (ddd)") }} - {{ convert_date_to_local_format(event.end_date, "ll (ddd)") }}</h3>
                                                <h4 class="time">{{ changeTimeFormat(event.start_time,'hh:mm A') }} - {{ changeTimeFormat(event.end_time, 'hh:mm A') }}</h4>
    ...
    methods: {
        ...
        checkSeatAvailability(schedule_date, $this) {
            ...
            if(total_capacity_temp > 0) {
                // check total_quantity for each ticket
                // and if total_vacant = 0 in all tickets
                ...
                for(var index in $this.tickets) {
                    ...
                    if (typeof($this.booked_tickets[ticket_id+'-'+schedule_date]) != "undefined") {
                        return_val = false;
                        if($this.booked_tickets[ticket_id+'-'+schedule_date].total_vacant > 0) {
                            // return true only if atleast one schedule is vacant
                            return_val = $this.booked_tickets[ticket_id+'-'+schedule_date].total_vacant;
                            // break the loop
                            return return_val;
                        } 
                    } else {
                        // e.g in case of booking for only one ticket out of 3 tickets
                        return_val = true;
                    }
                }
            } 

            // default return is very important!!!
            return return_val;
        },
