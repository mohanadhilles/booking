export default{
    ...
    methods: {
        ...
        convert_date_to_local(date) {
            ...
            // return moment(date).tz(local_tz);
        },

        // server date convert into local date
        convert_date_to_local_format(date, format) {
            var local_tz          = Intl.DateTimeFormat().resolvedOptions().timeZone; // local timezone
            if(typeof(date) == 'undefined' || date == null)
                return null;
            
            return moment(date).tz(local_tz).format(format);
        },
        ...
        dateToShortDate(date, monthYear) {
            return moment(moment(monthYear).format("YYYY-MM")+'-'+date).format("ll (ddd)")
        },
